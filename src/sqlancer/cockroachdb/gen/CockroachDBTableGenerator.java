    ...
    public void buildStatement() {
        ...
        for (int i = 0; i < columns.size(); i++) {
            ...
            if (generatedColumn) {
                ...
                errors.add("expected computed column expression");
                errors.add("volatile functions are not allowed in computed column");
                errors.add("expected STORED COMPUTED COLUMN expression to have");
            ...
            // Radar does not support default
            if (!generatedColumn && cockroachDBColumn.getType().getPrimitiveDataType() != CockroachDBDataType.SERIAL
                    && Randomly.getBoolean() && globalState.getDbmsSpecificOptions().getTestOracleFactory().get(0) != CockroachDBOptions.CockroachDBOracleFactory.EDC) {
                sb.append(" DEFAULT (");
                ...
                errors.add("expected DEFAULT (in CREATE TABLE) expression to have");
            }
