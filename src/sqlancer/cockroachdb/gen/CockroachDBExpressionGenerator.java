...
public class CockroachDBExpressionGenerator
    ...
    public CockroachDBExpression generateExpression(CockroachDBCompositeDataType type, int depth) {
        ...
        if (depth >= globalState.getOptions().getMaxExpressionDepth() || Randomly.getBoolean()) {
            ...
            if (Randomly.getBooleanWithRatherLowProbability()) {
                List<CockroachDBFunction> applicableFunctions = CockroachDBFunction.getFunctionsCompatibleWith(globalState, type);
    ...
    protected CockroachDBExpression generateColumn(CockroachDBCompositeDataType type) {
        ...
        if(globalState.getDbmsSpecificOptions().getTestOracleFactory().get(0) == CockroachDBOptions.CockroachDBOracleFactory.EDC) {
            if(column.getName().contains("rowid")){ //we do not generate rowid cause equdb cannot copy rowid values
                boolean found = false;
                for (int i = 0; i < columns.size(); i++) { //
                    if (columns.get(i).getType() == type && !columns.get(i).getName().contains("rowid")) {
                        found = true;
                        break;
                    }
                }
