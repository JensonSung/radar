...
public final class SQLite3RandomQuerySynthesizer {
    ...
    public static SQLite3Expression generate(SQLite3GlobalState globalState, int size) {
        ...
        SQLite3ExpressionGenerator aggregateGen;
        if (globalState.getDbmsSpecificOptions().oracles == SQLite3Options.SQLite3OracleFactory.EDC) { // do not generate aggregate function for sqlite in CREATE VIEW
            aggregateGen = new SQLite3ExpressionGenerator(globalState).setColumns(s.getTables().getColumns()).deterministicOnly();
        } else {
            aggregateGen = new SQLite3ExpressionGenerator(globalState).setColumns(s.getTables().getColumns()).allowAggregateFunctions();
        }
        ...
        for (int i = 0; i < size; i++) {
            if (Randomly.getBooleanWithRatherLowProbability() && globalState.getDbmsSpecificOptions().oracles != SQLite3Options.SQLite3OracleFactory.EDC) {
