...
public enum CockroachDBFunction {
    ...
    public static List<CockroachDBFunction> getFunctionsCompatibleWith(CockroachDBProvider.CockroachDBGlobalState state, CockroachDBCompositeDataType returnType) {
        List<CockroachDBFunction> candidateFunctions = new ArrayList<>();
        for (CockroachDBFunction function : values()) {
            if (function.isCompatibleWithReturnType(returnType)) {
                candidateFunctions.add(function);
            }
        }
        if (state.getDbmsSpecificOptions().getTestOracleFactory().get(0) == CockroachDBOptions.CockroachDBOracleFactory.EDC) {
            candidateFunctions.remove(CURRENT_DATABASE);
            candidateFunctions.remove(CURRENT_SCHEMA);
        }
