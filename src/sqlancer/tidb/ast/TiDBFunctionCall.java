...
//        IS_IPV6(1),
        ...
        public static TiDBFunction getRandom(TiDBProvider.TiDBGlobalState state) {
            List<TiDBFunction> candidates = new ArrayList<>(Arrays.asList(values()));
            if (state.getDbmsSpecificOptions().getTestOracleFactory().get(0) == TiDBOptions.TiDBOracleFactory.EDC) {
                candidates.remove(CONNECTION_ID);
                candidates.remove(DATABASE);
                candidates.remove(SCHEMA);
            }
            while (true) {
                TiDBFunction func = Randomly.fromList(candidates);
                if (func.getNrArgs() != -1) {
                    // special functions that need to be created manually (e.g., DEFAULT)
                    return func;
                }
