...
public class SQLite3Provider extends SQLProviderAdapter<SQLite3GlobalState, SQLite3Options> {
    ...
    public void generateDatabase(SQLite3GlobalState globalState) throws Exception {
        ...
        if (globalState.getDbmsSpecificOptions().generateDatabase) {
            ...
            int nrTablesToCreate = Randomly.fromOptions(1, 2, 3);
...
//            }
    ...
    private SQLQueryAdapter getTableQuery(SQLite3GlobalState globalState, int i) throws AssertionError {
        ...
        if (!globalState.getDbmsSpecificOptions().testFts || globalState.getDbmsSpecificOptions().getTestOracleFactory().get(0) == SQLite3OracleFactory.EDC) {
            options.remove(TableType.FTS);
        }
        if (!globalState.getDbmsSpecificOptions().testRtree || globalState.getDbmsSpecificOptions().getTestOracleFactory().get(0) == SQLite3OracleFactory.EDC) {
    ...
    private void addSensiblePragmaDefaults(SQLite3GlobalState globalState) throws Exception {
        ...
        if (Randomly.getBoolean() && globalState.getDbmsSpecificOptions().oracles != SQLite3OracleFactory.PQS && globalState.getDbmsSpecificOptions().oracles != SQLite3OracleFactory.EDC) {
            // the PQS implementation currently assumes the default behavior of LIKE
            pragmasToExecute.add("PRAGMA case_sensitive_like=ON;");
        }
        if (Randomly.getBoolean() && globalState.getDbmsSpecificOptions().oracles != SQLite3OracleFactory.PQS && globalState.getDbmsSpecificOptions().oracles != SQLite3OracleFactory.EDC) { // avoid inconsistent behaviors
