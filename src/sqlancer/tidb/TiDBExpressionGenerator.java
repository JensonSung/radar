...
public class TiDBExpressionGenerator extends UntypedExpressionGenerator<TiDBExpression, TiDBColumn> {
    ...
    protected TiDBExpression generateExpression(int depth) {
        ...
        List<Gen> candidates = new ArrayList<>(Arrays.asList(Gen.values()));
        if(globalState.getDbmsSpecificOptions().getTestOracleFactory().get(0) == TiDBOptions.TiDBOracleFactory.EDC) {
            candidates.remove(Gen.DEFAULT); // EDC does not support default function
            candidates.remove(Gen.REGEX); // too much regex errors
        }
        switch (Randomly.fromList(candidates)) {
        case DEFAULT:
            if (TiDBBugs.bug15) {
        ...
        case FUNCTION:
            TiDBFunction func = TiDBFunction.getRandom(globalState);
