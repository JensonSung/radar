...
public class SQLite3PragmaGenerator {
    ...
    public SQLQueryAdapter insert(SQLite3GlobalState globalState) {
        ...
        List<Pragma> candidates = new ArrayList<>(List.of(Pragma.values()));
        if (globalState.getDbmsSpecificOptions().getTestOracleFactory().get(0) == SQLite3Options.SQLite3OracleFactory.EDC) {
            candidates.remove(Pragma.REVERSE_UNORDERED_SELECTS); // avoid ambitious with select distinct, https://sqlite.org/forum/forumpost/b49034dbec
        }
        Pragma p = Randomly.fromList(candidates);
        switch (p) {
        case APPLICATION_ID:
